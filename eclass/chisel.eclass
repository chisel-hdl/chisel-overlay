...
PKGSRCDIR=${WORKDIR}/${P}/src/main/scala
...
chisel_src_compile() {
	mkdir -p ${PKGBUILDDIR}
	chiselc ${PKGSRCDIR} ${PKGBUILDDIR} --portagePkgDepends "${CHISEL_LIBRARY_DEPENDENCIES}" --portagePkgDbDir ${EPREFIX}/var/db/pkg --portagePkgJarDir ${EPREFIX}/usr/share/chisel/libs --scalacOpts ${SCALACOPTS} --outputJar ${PKGBUILDDIR}/${P}.jar || die "chiselc failed"
}
